...
def count_pairs(ids, weights=1, counts=None):
    if counts is None:
...
def find_chunk_boundaries(file_path, num_chunks, end_token="<|endoftext|>"):
    ...
    with open(file_path, "rb") as file:
        ...
        chunk_boundaries.append(file_size) # ファイル終端を追加
        
        buffer_size = 4096 # 境界から先読みするバイト数(仮想メモリの単位)

        # 境界位置の調整(終了トークンを探す)
        # 現在の境界位置から右にbuffer_size分ずらしながら、<|endoftext|>で区切れる境界を探す
        for bi in range(1, len(chunk_boundaries) - 1):
            chunk_position = chunk_boundaries[bi]
            file.seek(chunk_position) # 境界の推定位置からの開始
            ...
            while True:
                ...
                # 読み取ったチャンクで終了トークンを検索
                # end_token が text_bytes の何番目の位置から始まっているかを取得
...
def train_bpe(file_path, vocab_size, end_token="<|endoftext|>"):
    ...
    with open(file_path, "rb") as f:
        total_chunks = len(chunk_boundaries) - 1 # chunk_boundariesはファイル終端も含むため-1をする
